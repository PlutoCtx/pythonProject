...
def print_book_info():
    ...
    for key in book_info:
        book = book_info[key]
        print('%d\t\t%s\t\t%s\t%d' % (key, book['name'], book['author'], book['num']))
...
def search_book_info():
    ...
    if book_stock.get(book_no) is not None:
        print('书号\t\t书名\t\t作者\t\t价格\t\t库存数')
        print(book_no, '   ' + book_info[book_no]['name'], '   ' + book_info[book_no]['author'],
              '    ' + str(book_info[book_no]['num']), '    ' + str(book_stock[book_no]))
    else:
        print("书号不存在")
